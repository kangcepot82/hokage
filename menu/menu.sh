#!/bin/bash
BURIQ () {
    curl -sS https://raw.githubusercontent.com/kangcepot82/ipvps/main/ijin > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/kangcepot82/ipvps/main/ijin | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/kangcepot82/ipvps/main/ijin | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/kangcepot82/ipvps/main/ijin | grep $MYIP | awk '{print $3}')
fi
...
export sem=$( curl -s https://raw.githubusercontent.com/kangcepot82/ipvps/main/versi)
...
echo -e "              ${BICyan}${On_IPurple} SCRIPT MULTI VER 4.0 STABILE ${UWhite}${NC}"
echo -e "${BICyan} ┌─────────────────────────────────────────────────────┐${NC}"
...
9) clear ; genssl ;;
10) clear ; jam ;;
6969) clear ; wget https://raw.githubusercontent.com/kangcepot82/hokage/main/update.sh && chmod +x update.sh && ./update.sh && rm -f /root/update.sh ;;
0) clear ; menu ;;
x) exit ;;
